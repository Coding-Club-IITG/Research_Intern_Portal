    ...
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg mt-6 border border-gray-300">
      <div className="flex items-center mb-4">
        <div className="w-32 h-32 rounded-full overflow-hidden bg-gray-200 flex items-center justify-center mr-6">
          <img src={profileData.profilePicUrl} alt="Profile Pic" className="object-cover w-full h-full"/>
        </div>
        <div>
          <h1 className="text-3xl font-semibold text-gray-800">{profileData.name}</h1>
          <p className="text-gray-600 mt-1 bg-gray-100 inline-block p-1 rounded-md">{profileData.activityStatus}</p> {/* Active Today */}
          <p>{profileData.location} • {profileData.timezone}</p> {/* Location and Timezone */}
        </div>
        <div className="ml-auto flex space-x-4">
          <a href={profileData.linkedinUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-800">
            <img src="https://img.icons8.com/ios-filled/50/000000/linkedin.png" alt="LinkedIn" className="w-6 h-6"/>
          </a>
          <a href={profileData.resumeUrl} target="_blank" rel="noopener noreferrer" className="text-gray-700 hover:text-gray-900">
            <img src="https://img.icons8.com/ios-filled/50/000000/resume.png" alt="Resume" className="w-6 h-6"/>
          </a>
        </div>
      </div>
      ...
      <div className="bg-white p-2 rounded-lg mb-2">
        ...
        <p className="text-gray-600">{profileData.education.institution} • {profileData.education.graduationYear}</p>
      </div>
